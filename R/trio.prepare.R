...
function(trio.dat, freq=NULL, blocks=NULL, logic=TRUE, ...){
   if(!is.null(blocks)){
	blocks <- NULL
	cat("NOTE: The specification of blocks is currently ignored, as the package\n",
	"haplo.stats is currently not available at CRAN. Thus, all SNPs are treated\n",
	"as if they were not in LD, even if the argument blocks is specified.\n\n",
	"Haplotypes and their frequencies can still be taken into acount\n",
	"when generating the pseudo-controls by specifying the argument freq.\n\n",
	sep = "")
   }
