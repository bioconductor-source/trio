...
\texttt{haplo.stats} package.

\textbf{Since the haplo.stats package is currently not available at CRAN, the argument blocks is currently ignored so that no haplotype frequencies are estimated using haplo.em and no haplotype information is employed in the generation of the pseudo-controls. See Section \ref{haplotypeFreq} for how the argument freq of the function trio.prepare can be used to specify haplotypes and their frequencies for the generation of the pseudo-controls}.

\textbf{Since we guess that the unavailability of haplo.stats is only temporary, we have not changed yet the discussion in the following sections on how the the specification of the argument blocks is used in the generation of the pseudo-controls. However, please note that in the application of trio.prepare this argument is ignored and the SNPs are considered individually, i.e.\ as if they were not in a haplotype block, even though the discussion states otherwise.}
...
\subsection{Using Haplotype Frequencies}\label{haplotypeFreq}
...
in the stochastic search algorithms, simulated annealing and MCMC, employed in trio logic regression. \textbf{In a real trio
...
<<>>=
fs.out <- trioFS(trio.bin, B=5, control=my.control, rand=1234)
fs.out
@
